/**
 * @file    asm_spi_test.c
 * @brief   BT892X 软件 SPI 汇编优化对比
 *
 *  对比 C 版本 vs 汇编版本的 soft_spi_byte 速度
...
/* ================================================================
 *  ASM 版本: 内联汇编, 展开循环, 直接 SFR 地址访问
 *  (delay_us 仍是 C 调用, 因为它是时序核心)
 * ================================================================ */
...
void asm_spi_test(void)
{
    ...
    // 对比 (用有符号避免回绕)
    printf("\n=== Comparison (unrolled C loop as baseline) ===\n");
    ...
    printf("Pure ASM file (not inline) could do better, but gains are small.\n");
    printf("Lesson: trust the compiler for simple GPIO; use ASM only if\n");
    printf("  you can merge delay+GPIO into a tight instruction sequence.\n");
